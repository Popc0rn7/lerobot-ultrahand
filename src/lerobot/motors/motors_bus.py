...
class MotorsBus(abc.ABC):
    ...
    def set_max_turn_homings(self, motors: NameOrID | list[NameOrID] | None = None) -> dict[NameOrID, Value]:
        """Centre each motor range around its current position.
        ...
        self.reset_calibration(motors)
        actual_positions = self.sync_read("Present_Position", motors, normalize=False)
        homing_offsets = self._get_max_turn_homings(actual_positions)
        for motor, offset in homing_offsets.items():
            self.write("Homing_Offset", motor, offset)
    ...
    @abc.abstractmethod
    def _get_max_turn_homings(self, positions: dict[NameOrID, Value]) -> dict[NameOrID, Value]:
        pass
